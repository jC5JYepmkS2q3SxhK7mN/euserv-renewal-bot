...
class RenewalBot:
    ...
    def send_telegram_message(self, subject_status: str) -> None:
        ...
        if not (TG_BOT_TOKEN and TG_CHAT_ID):
            self.log("Telegram通知所需的 TG_BOT_TOKEN 或 TG_CHAT_ID 未设置，跳过发送电报。")
            return
        
        self.log("正在准备发送 Telegram 状态通知...")
        subject = f"Euserv 续约脚本运行报告 - {subject_status}"
        body = f"{subject}\n\nEuserv 自动续约脚本本次运行的详细日志如下：\n\n" + "\n".join(
            self.log_messages
        )
        ...
        try:
            response = requests.post(api_url, json=payload, timeout=HTTP_TIMEOUT_SECONDS)
            response.raise_for_status()
            self.log("Telegram 状态通知已成功发送！", LogLevel.CELEBRATION)
        except requests.RequestException as e:
            self.log(f"发送 Telegram 通知失败: {e}", LogLevel.ERROR)
